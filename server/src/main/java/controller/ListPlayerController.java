...
public class ListPlayerController {
    ...
    public void acceptInviteFriend(ObjectWrapper objectWrapper) {
        ...
        for (SocketHandlers clientHandler : clients) {
            if (clientHandler.getLoginController().getPlayerLogin().getPlayerName()
                    .equals(friendInvite.getPlayer().getPlayerName())) {
                objectWrapper = new ObjectWrapper(msg,
                        socketHandlers.getLoginController().getPlayerLogin().getPlayerName());
    ...
    public void inviteFriend(ObjectWrapper objectWrapper) {
        ...
        for (SocketHandlers clientHandler : clients) {
            if (clientHandler.getLoginController() != null &&
                ...
                objectWrapper = new ObjectWrapper(StreamData.Message.SEND_INVITE_FRIEND.name(),
        ...
        for (SocketHandlers clientHandler : clients) {
            ...
            if (clientHandler.getLoginController() != null
                ...
                if (!playerLogin.getPlayerName().equals(player.getPlayerName())) {
                    PlayerStatus playerStatus = new PlayerStatus(player.getPlayerName(),
                            playerDAO.isFriend(player.getId(), playerLogin.getId()), player.getStatus());
        ...
        for (SocketHandlers clientHandler : clients) {
            ...
            for (SocketHandlers clientHandler2 : clients) {
                if (!clientHandler2.equals(clientHandler)
                    ...
                    if (!playerLogin.getPlayerName().equals(player.getPlayerName())) {
                        PlayerStatus playerStatus = new PlayerStatus(playerLogin.getPlayerName(),
                                playerDAO.isFriend(player.getId(), playerLogin.getId()), playerLogin.getStatus());
